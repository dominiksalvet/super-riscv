...
define HELP_MSG
SUPPORTED TARGETS:
    <none>/verilate       translate SV processor description into C++ model
    build                 compile C++ model into executable processor simulator
    sim                   compile RISC-V program and run processor simulation
                          with the program
    debug                 run processor simulation with RISC-V program and
                          create files for processor debugging
    open_waves            display signal waves generated by 'debug' target
...
ACCEPTED MACROS:
    ...
    TEST_NAME=<name>      test to be run on processor
    SIM_OUT_DIR=<path>    directory for all files generated by simulation run
